...
const projects = [
  {
    num: '01', border: 'border-t-blue-500', numColor: 'text-blue-500 dark:text-blue-400', gradient: 'from-blue-500/25 to-blue-500/5',
    icon: <FaCode className="text-blue-500 dark:text-blue-400" size={16} />,
  ...
  {
    num: '02', border: 'border-t-[#0EA5E9]', numColor: 'text-cyan-500 dark:text-cyan-400', gradient: 'from-cyan-500/25 to-cyan-500/5',
  ...
  {
    num: '03', border: 'border-t-purple-500', numColor: 'text-purple-500 dark:text-purple-400', gradient: 'from-purple-500/25 to-purple-500/5',
    icon: <FaCode className="text-purple-500 dark:text-purple-400" size={16} />,
  ...
  {
    num: '04', border: 'border-t-emerald-500', numColor: 'text-emerald-500 dark:text-emerald-400', gradient: 'from-emerald-500/25 to-emerald-500/5',
    icon: <FaCode className="text-emerald-500 dark:text-emerald-400" size={16} />,
  ...
  return (
    <SectionWrapper id="qa-projects" className="section-light">
      ...
      <div className="grid sm:grid-cols-2 gap-7">
        {projects.map(({ num, border, numColor, gradient, icon, title, tech, desc, testing, badge }, i) => (
          <motion.div
            ...
            whileHover={{ y: -8, border: '1px solid rgba(56,189,248,0.3)', boxShadow: '0 20px 50px rgba(56,189,248,0.08)' }}
            className={`relative bg-white/60 dark:bg-slate-900/40 rounded-3xl border-t-2 ${border} border-x border-b border-slate-200 dark:border-slate-800/80 p-7 flex flex-col gap-5 transition-all duration-300 shadow-md hover:shadow-lg group overflow-hidden`}
          >
            {/* Badge */}
            {badge && (
              <motion.div
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-start gap-3 mb-2">
                  <div className={`bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl flex items-center justify-center shrink-0 p-2 text-slate-800 dark:text-white`}>
                    {icon}
                  </div>
                  <h3 className="font-black text-slate-900 dark:text-white text-base leading-snug group-hover:text-[#38BDF8] transition-all duration-300">
                    {title}
                  </h3>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2">
              {tech.map((t) => (
                <span key={t} className="text-xs font-bold px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-950/50 text-[#2563EB] dark:text-[#38BDF8] border border-blue-500/10">
                  {t}
                </span>
              ))}
            </div>
            ...
            {/* Testing focus */}
            <div className="bg-slate-100/60 dark:bg-slate-950/40 rounded-2xl p-5 border border-slate-200 dark:border-slate-850">
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-slate-900 dark:text-white">Testing Focus</span>
              </div>
              <ul className="grid grid-cols-1 gap-2">
                {testing.map((t) => (
                  <li key={t} className="flex items-start gap-2.5 text-xs text-slate-600 dark:text-slate-400">
      ...
      <motion.div
        ...
        className="mt-16 bg-white/60 dark:bg-slate-900/40 border border-slate-200 dark:border-emerald-500/20 rounded-3xl p-8 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-xl"
      >
        ...
        <div className="relative z-10 flex-1">
          <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Prove Your QA Excellence</h3>
